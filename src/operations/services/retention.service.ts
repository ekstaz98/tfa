...
export class RetentionService {
  private readonly _logger = new Logger(RetentionService.name);
  ...
  constructor(
    ...
    this._retentionDays = config.getOrThrow<number>('retention.days');
  }
  ...
  async cleanup(): Promise<number> {
    ...
    const deleted = await this._operationsCrud.deleteFinishedBefore(cutoff);
    if (deleted > 0) {
      this._logger.log(
        `Retention: deleted ${deleted} operations older than ${this._retentionDays} days`,
      );
    }
    return deleted;
  }
