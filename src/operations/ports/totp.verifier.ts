...
import { base32Decode, currentTimeStep, totpCode } from '../totp';
...
export class TotpVerifier implements CodeVerifierPort {
  ...
  async verify(context: CodeVerifyContext): Promise<boolean> {
    ...
    const secret = base32Decode(this.cipher.decrypt(credential.secret));
    const lastUsed =
    ...
    for (const step of [now - 1, now, now + 1]) {
      ...
      if (
        ...
        await this.credentialsCrud.update(
          credential.id,
